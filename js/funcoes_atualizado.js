...
//------------------------Função Afastamentos (por mês/ano) -------------------------------------------------------------------

function adicionarAfastamento() {
    const mesAno = document.getElementById('mesAfastamento').value; // formato yyyy-mm
    ...
    if (existe) {
        alert("Já existe afastamento lançado para este mês. Remova antes de adicionar novamente.");
        return;
    }
...
function atualizarResumoAfastamento() {
    ...
    resumoDiv.innerHTML = '<h4>Afastamentos:</h4>';
...
// -------------------------Funções para Gerar Carreira (versão mensal) ----------------------------------
...
function gerarCarreira() {
    ...
    listarAperfeicoamentos.forEach(({ data, pontuacao }) => {
        const [ano, mes] = data.split("-"); // data yyyy-mm-dd
        const chave = `${ano}-${mes}`;
    ...
    // Primeira linha apenas com pontuação remanescente
    linhasTabelaHTML += `
        <tr>
            <td>${formatarDataBR(dataAtual)}</td>
            <td>-</td>
            <td>-</td>
            <td>-</td>
            <td>-</td>
    ...
    for (let i = 0; i < 400; i++) {
        ...
        if (dia === 1) {
            ...
            diasAfastados = listaAfastamentos.find(a => a.mesAno === chaveAnterior)?.quantidade || 0;
